...
@jit(nopython=True, nogil=True, fastmath=True)
def grow_cell(cloud_id, mask, uid, base, top, i, j, t, nt, itot, jtot):
    """
...
def find_cells(cloud_id, mask, base, top, nt, itot, jtot):
    """
        ...
        for j in range(jtot):
            for i in range(itot):
                if mask[t,j,i] and cloud_id[t,j,i] == 0:
                    grow_cell(cloud_id, mask, uid, base, top, i, j, t, nt, itot, jtot)
                    uid += 1
...
if __name__ == '__main__':
    ...
    find_cells(cloud_id, wp_mask,  base, top, nt, itot, jtot)
    print('Tracking cores')
    find_cells(core_id,  thv_mask, base, top, nt, itot, jtot)
